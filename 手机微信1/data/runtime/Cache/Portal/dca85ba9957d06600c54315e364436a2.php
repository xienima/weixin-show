...
<div id="frame">
	...
	<div id="top">
		...
		<span style="color: #FF5400">公司新闻</span>
...
<!-- 	<div id="con">
		...
		<?php $lists = sp_sql_posts_paged("cid:4,field:post_title,post_content,smeta;order:post_date DESC;",10); ?>
		<?php if(is_array($lists['posts'])): foreach($lists['posts'] as $key=>$vo): $smeta=json_decode($vo['smeta'], true); ?>
			<!--循环开始-->
				<div  style="float:left;width:100%;height:105px;margin-left:2%;overflow:hidden;margin-top:10px;">
					<a href="<?php echo leuu('article/index',array('id'=>$vo['tid']));?>">
					<div style="width: 30%;height:75px;margin-top:10px;margin-left:15px;float: left">
					<img src="<?php echo sp_get_asset_upload_path($smeta['thumb']);?>" style="width: 100%;height:100%;"></div>

					<div style="width:55%;height:75px;float: left;margin-left:  15px;margin-top:10px;text-align:center;line-height:75px;">
...
<style>
.pagination li{float: left;list-style: none;margin-left: 15px}
